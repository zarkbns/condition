...
- **Preprod (current persistent testnet):** indexer `https://indexer.preprod.midnight.network/api/v3/graphql` (WS `…/api/v3/graphql/ws`) — v3 is the surface this repo's SDK generation is verified against (see `docs/DEPLOYMENTS.md`); node `https://rpc.preprod.midnight.network`, faucet `https://faucet.preprod.midnight.network`. No hosted prover — the CLI runs a local Docker proof server (`localhost:6300`); the browser path needs none because proving is delegated to the wallet. Set `MIDNIGHT_NODE_URL` in `.env`. The old testnet-02 network is retired and its endpoints no longer resolve.
...
- **Wallets:** two paths, one `LiveStack` surface. CLI/deployer: seed-backed facade wallet (`MIDNIGHT_WALLET_SEED`, never in the browser bundle). Browser: the Midnight **DApp Connector** (`src/utils/laceConnector.ts`) — wallets self-inject under `window.midnight.{id}`; page load only *discovers* them, and the connect button calls `connectBrowserStack()` on a user gesture. A v4 connector supplies proving (`getProvingProvider`), balancing/signing (`balanceUnsealedTransaction`) and relay (`submitTransaction`), so the site holds no keys. Version/capability probes fail loud rather than falling back. The services API stays wallet-agnostic on purpose.
