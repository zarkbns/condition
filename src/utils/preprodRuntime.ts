...
export class PreprodConditionRuntime implements AsyncConditionRuntime {
  ...
  /**
   * Adopt the CLAIM half of a split e2e run: the enrollment and trigger
   * already happened on-chain in a previous process; here we only replay
   * them on the local mirror so settle() has the full policy state, and
   * re-register the holder secret so the on-chain settle witnesses can be
   * constructed. No transactions are fired.
   *
   * The holderSecret is the one piece of private material a split e2e must
   * hand between processes (generated by the claim stage, consumed here).
   * It travels via a mode-0600 local file controlled by the operator —
   * this is TEST-ONLY plumbing for scripts/e2e-preprod.ts; the product
   * path never serializes the secret anywhere (Invariant 2).
   */
  async adoptClaim(input: {
    policyId: Bytes32;
    commitment: Bytes32;
    premium: Dust;
    enrolledAt: number;
    ...
    this.privateLedger_.enroll(input.policyId, input.holderSecret);
    this.onChainClient_.registerHolderSecret(input.policyId, input.holderSecret);
    await this.local.policyService.publishEnrollment(
      input.policyId, input.commitment, input.premium, input.enrolledAt,
    );
