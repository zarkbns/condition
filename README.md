...
The "live end-to-end" capabilities in this README are CLI capabilities. The deployed frontend (Vercel) showcases the protocol — pages, client-side proof generation, receipt verification, and the LOCAL DEV reference loop. A third path — real Preprod writes from a visitor's browser via the Midnight DApp Connector — is wired (`src/utils/laceConnector.ts`): connect → prepare transaction → prove → sign → submit → confirm, with **proving delegated to the wallet** (`getProvingProvider`, connector API v4) and the zk circuit artifacts served from the site itself. The adapter path is verified end-to-end on Preprod by `scripts/probe-browser-stack.ts` (real proofs via a proof server, real wallet balancing/signing behind the connector wire format, real confirmed transactions); the one surface it cannot exercise headlessly is the Lace extension UI itself, so treat the in-browser flow as **wired, verified at the adapter level, pending a live Lace click-through**:
...
| Midnight DApp Connector (Lace) in the browser | **Browser** — `window.midnight` discovery + v4 capability probe; connecting builds the live on-chain stack (fail loud on missing capabilities, never simulate) |
| Proof server (`http://127.0.0.1:6300`) | **Developer-local only** — required by the CLI deployer/e2e/adapter-probe for contract proving. There is no hosted Preprod prover, a Vercel visitor cannot reach the developer's localhost, and the deployed frontend neither uses nor depends on it: browser proving is **delegated to the wallet** (`getProvingProvider`), with only public circuit artifacts (`keys/`, `zkir/` — compiler outputs, no witness data) served from the site |
...
The browser bundle is audited to contain no Node-only modules, no seed handling, and no secrets. The connector and proving packages are browser-safe and loaded only through dynamic imports, so they stay out of the first-paint chunk and are pulled in solely once a wallet connection is actually attempted.

**What a Vercel visitor can and cannot do:** on page load the frontend only *discovers* wallets injected under `window.midnight` — it never prompts silently. Pressing **Connect Wallet** (a user gesture) opens the Midnight DApp Connector and, on a connector-API-v4 wallet, builds the real on-chain stack: transactions are prepared in the page, proofs are generated **by the wallet**, and the wallet balances, signs and submits. So a visitor with the Lace extension can run real Preprod writes without handing this site a seed or a private key. If the wallet is missing, refuses, or lacks proving capability, the path fails loud with the precise reason — it never degrades into a simulated connection or a fake transaction. The CLI path (`npm run deploy`, `scripts/e2e-preprod.ts`) remains the seed-backed route on the developer's machine. The proof server at `http://127.0.0.1:6300` is that route's local dependency only: there is no hosted Preprod prover, a visitor cannot reach the developer's localhost, and the deployed frontend neither uses nor needs it — browser proving is delegated to the wallet, and the site serves nothing but the circuits' public compiler outputs (`frontend/public/contracts`).
